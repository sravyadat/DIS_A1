        ...
        private static string RemoveVowels(string s)
        {
            try
            {
                ...
                {//Checking Vowels in string 's'
                    ...
                    {
                        final_string += "";  // any vowels are present in string 's' are skipped
                    }
                    else
                    {
                        final_string += s[i]; //no vowels are present ,characters in string are concatenated
                    }
                }
                return final_string; // returning final output
            }
            catch (Exception)
            {
                throw;
       ...
       */
        private static bool ArrayStringsAreEqual(string[] bulls_string1, string[] bulls_string2)
        {
            try
            {
                ...
                {
                    s2 = s2 + bulls_string2[j]; //all characters or string present in the index
                }

                if (s1 == s2) //checking if all are equivalent
                {
                    return true; //Returns 
                }
        ...
        */
        private static int SumOfUnique(int[] bull_bucks)
        {
            try
            {
                ...
                {
                    if (d.ContainsKey(i))  // check the presence of the number 
                    {
                        d[i] = d[i] + 1;  // multiple values, dictionary 'd[i]' is re-incremented as key is same 
                    }
                    else
                    {
                        d[i] = 1;
                ...
                foreach (var a in d) //iteration sum the unique elements
                {
                    if (a.Value == 1)
                    {
                        sum = sum + a.Key; //unique elements added to integer 'sum' in each iteration

                           
                    }
                }

                return sum;  //returning final output the sum of unique elements
            }
            catch (Exception)
            {
                throw;
        ...
        */
        private static string ReversePrefix(string bulls_string6, char ch)
        {
            try
            {
                ...
                int i = 0, j = 0;
                //Take integer variable 'j' to get thee index 
                for (i = 0; i < bulls_string6.Length; i++)
                {
                    if (ch == bulls_string6[i])// Check if character is there or not
                    {
                        j = i;// Character is present means the index of ten character will be assigned to j.
                    }
